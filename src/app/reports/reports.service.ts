...
export class ReportsService {
  ...
  private processEventLogs:boolean = false;
  ...
  constructor(private eventLoggerService: EventLoggerService) {
    ...
    this.eventLoggerService.eventLogs$
      .pipe(filter(() => this.processEventLogs))
      .subscribe((eventLogs) => {
  ...
  set setProcessEventLogs(processEventLogs: boolean) {
    this.processEventLogs = processEventLogs;
  }
  ...
  processSubgroup(
    subgroups: RawEventAndDateTypePerSubGroupIdAndDate | undefined,
    eventLog: EventLog,
    subgroupId: number,
    ...
    if (!newSubGroups[subgroupId]) {
      newSubGroups[subgroupId] = { name: subgroupNameOrDescription };
    }

    const subgroup = newSubGroups[subgroupId];
  ...
  generateReportTotalHoursPerGroup(eventLogAttributesFilter: EventLogAttributesFilter|undefined) {
